...
For organizations that want to avoid transmitting data externally, OpenCode also provides a [local sharing](/docs/local-share) feature that generates HTML files on the user's computer without uploading to any remote service.

We recommend you disable remote sharing for your trial while allowing local sharing for internal collaboration:
...
Users can then use `/share-local` to generate HTML files that can be shared internally through approved channels.

[Learn more about sharing](/docs/share) and [local sharing](/docs/local-share).
