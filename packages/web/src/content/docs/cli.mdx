...
For sharing conversations, OpenCode provides both [remote sharing](/docs/share) and [local sharing](/docs/local-share) options through CLI commands.
...
Generate a local HTML share of a session without publishing online.

```bash
opencode share-local [session]
```

Creates an HTML file in your project's `.opencode/shares/` directory containing the conversation that can be opened in any web browser. The file path is copied to your clipboard upon successful generation.
...
| Flag     | Short | Description                                  |
| -------- | ----- | -------------------------------------------- |
| `--open` | `-o`  | Open the generated HTML file in your browser |

#### Examples

```bash
# Generate local share for current or last session
opencode share-local

# Generate local share for specific session
opencode share-local session_abc123def456

# Generate local share and open in browser
opencode share-local session_abc123def456 --open
```
